...
WorkBuddy setup preserves unrelated entries in `~/.workbuddy/mcp.json` and creates a timestamped backup before mutation. Inside the WorkBuddy sandbox it selects a session-managed Local Preview receiver, so first pairing does not require Terminal or `launchctl`. It opens the loopback pairing page immediately; the QR advertises the Mac's LAN address, while MCP approval remains mandatory before any health data is read. It never edits WorkBuddy's approval store directly.
