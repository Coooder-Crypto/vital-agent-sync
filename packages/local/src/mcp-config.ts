...
function parseJsonRecord(value: string, owner: string): Record<string, unknown> {
  try {
    ...
    throw new Error(`${owner} config must be valid JSON for automatic Vital Agent Sync install. Use print-agent-config to merge manually if the config uses comments or JSON5 syntax. ${detail}`);
  }
}
...
function uniqueBackupPath(configPath: string): string {
  ...
  throw new Error("Could not allocate a unique Vital Agent Sync config backup path.");
}
