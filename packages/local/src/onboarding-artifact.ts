...
<head>
  ...
  <title>HealthLink iOS onboarding</title>
  <style>
...
<body>
  <main>
    <h1>Connect HealthLink iOS</h1>
    <p class="warning">This local page contains pairing credentials. Do not share or upload it.</p>
    ${showQr ? `<img src="${options.qrDataUrl}" alt="HealthLink onboarding QR code">` : ""}
    ${showDeepLink ? `<div><a href="${link}">Open HealthLink</a></div>` : ""}
    ${showText ? `<textarea readonly aria-label="HealthLink onboarding text code">${escapeHtml(options.textCode)}</textarea>` : ""}
